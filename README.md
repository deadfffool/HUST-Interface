...
步骤1. 通过将mie CSR中的位miep清零，禁止所有外部中断。
1. 通过写入mpiccfg寄存器的priord位来配置优先级顺序。
1. 在多向量模式下，如果未配置，则通过写入meivt寄存器的基址字段来设置外部向量中断地址表的基址。
1. 通过写入meipt寄存器的prithresh字段来设置优先级阈值。
1. 通过向meicidpl寄存器的clidpri字段和meicurpl寄存器的currpri字段写入“0”（或对于反转的优先级顺序写入“15”）来初始化嵌套优先级阈值。
1. 对于每个可配置的网关S，在meigwctrlS寄存器中设置极性（高电平有效/低电平有效）和类型（电平触发/边沿触发），并通过写入网关的meigwclrS寄存器将IP位清零。
1. 在多向量模式下，对于每个外部中断源S，将相应处理程序的地址写入外部向量中断地址表中。
1. 通过写入meiplS寄存器的相应优先级字段，设置每个外部中断源S的优先级。
1. 通过设置每个中断源S的meieS寄存器的inten位，为适当的外部中断源允许中断。
1. 激活mstatus CSR中的mei位。
1. 通过设置mie CSR中的位miep，允许所有外部中断。
